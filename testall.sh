...
# Regression testing script for MMM
...
Check() {
    ...
    clang++ `pkg-config --cflags opencv` `pkg-config --libs opencv` "${basename}.s" io.cpp -o "${basename}.exe" &&
...
CheckFail() {
    ...
    RunFail "$MMM" "<" $1 "2>" "${basename}.err" ">>" $globallog &&
